# Лабораторная работа №7А. Интеграция Watson Assistant и телефонии с использрванием voximplant
  ...
  - [1. Настройка Voximplant](#установка-node-red)
...
Текст сценария - см. внизу. Обратите внимание - переменные
var workspaceId;
Должны быть скопированы из Вашей конфигурации Watson Assistant.

Значение переменной **base64header** должно быть инициализировано из userid/password:
...
function sendToBot(text) {
    ...
    var base64header="OGM2MGI3NmUtNTgwYy00YzUyLWE5MzQtOTVkNjcwMmI4MDlhOk5TT0lMa0tCTWQyaA==";

    authorizationHeader="Authorization: Basic "+base64header;

    var serviceURL="https://watson-api-explorer.mybluemix.net/assistant/api/v1/workspaces/"+workspaceId+"/message?version=2018-02-16&nodes_visited_details=false"
...
Создайте приложение orderPizzaApp
...
Приобретите телефонный номер, к которому будет привязан Ваш чат-бот. Позднее Вам придется пройти верификацию, чтобы использовать номера на срок более двух недель. (Номера - купить номер телефона)

В списке номеров телефонов привяжите приложение "orderpizzaapp" и правило "ЧатБот":

<img src="images/7A-vox-linkrule.png" width=200 />


## 1.4. Протестируйте бота
